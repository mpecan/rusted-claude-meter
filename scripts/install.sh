...
echo "If you see 'Could not create surfaceless EGL display: EGL_BAD_ALLOC' or a blank"
echo "Settings/popover window, it's usually a stale libjxl on this host (the AppImage"
echo "bundles its own webkit build). Update libjxl (e.g. 'sudo pacman -Syu' on Arch)"
echo "and retry. WEBKIT_DISABLE_DMABUF_RENDERER=1 is a fallback that unblocks startup"
echo "but does not fix blank windows on its own: ${INSTALL_DIR}/${BIN_NAME}"
